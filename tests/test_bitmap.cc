...
#include "helens/ds/bitmap.h"
#include "helens/ds/roaring_bitmap.h"
...
void init(uint32_t size, std::set<uint32_t> v = {}) {
    ...
    std::set<uint32_t> v0;
    helens::ds::Bitmap::ptr b;
    if(!size) {
        size = rand() % 4096;
        b.reset(new helens::ds::Bitmap(size));
    ...
    helens::ds::Bitmap::ptr c(new helens::ds::Bitmap(size, 0xFF));
    *c &= *b;
    HELENS_ASSERT(c->getCount() == v0.size());
    ...
    //};
    //uint32_t size = 328;
    //helens::ds::Bitmap::ptr b(new helens::ds::Bitmap(size));
    //for(auto& i : vv) {
    //    b->set(i, true);
    //}
    ...
    helens::ds::Bitmap::ptr bb(new helens::ds::Bitmap(size, 0xFF));
    *b &= *bb;
#define XX(v) \
        HELENS_LOG_INFO(g_logger) << #v ": " << helens::Join(v.begin(), v.end(), ",");
...
#define XX_ASSERT(a, b) \
    ...
    if(v2 != v4 || v1 != v3) {
        ...
        XX_ASSERT(v00, v3);
        XX_ASSERT(v000, v4);
        //HELENS_ASSERT(v00 == v3);
        //HELENS_ASSERT(v1 == v3);
        //HELENS_ASSERT(v2 == v4);
#undef XX_ASSERT
#undef XX
    }
    //HELENS_LOG_INFO(g_logger) << "size: " << size;
}
...
void test_compress(size_t size, std::set<uint32_t> v = {}) {
    ...
    if(*b != *bb) {
        HELENS_LOG_INFO(g_logger) << "b  :" << b->toString();
        HELENS_LOG_INFO(g_logger) << "bc :" << bc->toString();
        HELENS_LOG_INFO(g_logger) << "bb :" << bb->toString();
        HELENS_LOG_INFO(g_logger) << "size=" << size << " - " << helens::Join(v.begin(), v.end(), ",");
...
void test_op(size_t size, std::set<uint32_t> v1, std::set<uint32_t> v2) {
    helens::ds::Bitmap::ptr a;
    helens::ds::Bitmap::ptr b;

    if(size == 0) {
        size = rand() % 4096;
        a.reset(new helens::ds::Bitmap(size));
        b.reset(new helens::ds::Bitmap(size));
    ...
    HELENS_ASSERT(a->cross(*b) == a->cross(*bc));
    ...
    HELENS_ASSERT(a->cross(*b) == (!and_sv.empty()));
...
int main(int argc, char** argv) {
    srand(time(0));
    {
        helens::ds::Bitmap::ptr b(new helens::ds::Bitmap(504));
    }
...
std::vector<helens::ds::Bitmap::ptr> vs;
std::vector<helens::ds::Bitmap::ptr> vs2;
std::vector<helens::ds::RoaringBitmap::ptr> vs3;
...
void init() {
    ...
    vs3.resize(N);
    for(int i = 0; i < N; ++i) {
        helens::ds::Bitmap::ptr r(new helens::ds::Bitmap(M));
        helens::ds::RoaringBitmap::ptr r2(new helens::ds::RoaringBitmap());
        for(int i = 0; i < M / 1; ++i) {
            uint32_t v = rand() % M;
            r->set(v, true);
            r2->set(v, true);
...
void write_to_file(const std::string& name) {
    {
        ...
        a.reset(new helens::ds::Bitmap(0));
        a->readFrom(ba);
...

    {
        ...
        a.reset(new helens::ds::RoaringBitmap);
        a->readFrom(ba);
...
void load_from_file(const std::string& name) {
    ...
    ba->setPosition(0);
    helens::ds::Bitmap::ptr a(new helens::ds::Bitmap(0));
    a->readFrom(ba);
    std::cout << "compress= " << a->isCompress()
              << " rate=" << a->getCompressRate()
              << " size=" << a->getSize()
...
void test_uncompress2() {
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            helens::ds::Bitmap::ptr b(new helens::ds::Bitmap(M));
            *b |= *vs2[i];
        }
    }
}
    ...
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            ...
            *b |= *vs2[i];
            ...
            HELENS_ASSERT(*b == *vs2[i]->uncompress());
        }
    }
}
...
void test_uncompress4() {
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            helens::ds::Bitmap::ptr b(new helens::ds::Bitmap(M, 0xff));
            *b &= *vs2[i];
...
void test_roaring_bitmap() {
    helens::ds::RoaringBitmap::ptr rb(new helens::ds::RoaringBitmap);
    for(int i = 0; i < 10; ++i) {
        rb->set(rand(), true);
    }
