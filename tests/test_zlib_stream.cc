...
void test_gzip() {
    ...
    std::string data = helens::random_string(102400);

    auto gzip_compress = helens::ZlibStream::CreateGzip(true, 1);
    ...
    auto gzip_uncompress = helens::ZlibStream::CreateGzip(false, 1);
...
void test_deflate() {
    ...
    auto gzip_compress = helens::ZlibStream::CreateDeflate(true, 1);
    std::cout << "compress: " << gzip_compress->write(data.c_str(), data.size())
              << " length: " << gzip_compress->getBuffers().size()
              << std::endl;
    std::cout << "flush: " << gzip_compress->flush() << std::endl;
    ...
    auto comperss_str = gzip_compress->getResult();
    auto gzip_uncompress = helens::ZlibStream::CreateDeflate(false, 1);
...
void test_zlib() {
    ...
    auto gzip_compress = helens::ZlibStream::CreateZlib(true, 1);
    std::cout << "compress: " << gzip_compress->write(data.c_str(), data.size())
              << " length: " << gzip_compress->getBuffers().size()
              << std::endl;
    std::cout << "flush: " << gzip_compress->flush() << std::endl;
    ...
    auto comperss_str = gzip_compress->getResult();
    auto gzip_uncompress = helens::ZlibStream::CreateZlib(false, 1);
