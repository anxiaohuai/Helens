...
void test_aes256ecb() {
    for(int i = 0; i < 4097; ++i) {
        auto str = helens::random_string(i);
        std::string encode;
        encode.resize(i+ 30);
        std::string decode;
        decode.resize(i+ 30);

        auto key = helens::random_string(32);

        int32_t len = helens::CryptoUtil::AES256Ecb(key.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if(len > 0) {
            encode.resize(len);
        }
        len = helens::CryptoUtil::AES256Ecb(key.c_str(), encode.c_str(), encode.size(), &decode[0], false);
...
void test_aes256cbc() {
    for(int i = 0; i < 4097; ++i) {
        ...
        auto key = helens::random_string(32);
        auto iv = helens::random_string(16);

        int32_t len = helens::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if(len > 0) {
            encode.resize(len);
        }
        len = helens::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
        std::cout << "decode_len: " << len << std::endl;
        if(len > 0) {
            decode.resize(len);
        }
...
void test_one() {
    //std::string key = helens::random_string(32);
    //std::string iv = helens::random_string(16);
    //std::string str = helens::random_string(125);
    ...
    int32_t len = helens::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
    if(len >= 0) {
        encode.resize(len);
    }

    int32_t len2 = helens::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
    if(len2 >= 0) {
        decode.resize(len2);
    }
    ...
    std::cout << "encode: " << helens::base64encode(encode) << " - len=" << encode.size() << std::endl;
...
void test_rsa() {
    ...
    //std::cout << "generate: " << helens::RSACipher::GenerateKey(pubkey_path, prikey_path) << std::endl;
    helens::RSACipher::ptr rsa = helens::RSACipher::Create(pubkey_path, prikey_path);
    if(!rsa) {
...
int main(int argc, char** argv) {
    ...
    //    auto v = helens::random_string(128, "0123456789");
    //    std::cout << "[" << v << "]" << std::endl;
    //    for(auto& n : v) {
    //        HELENS_ASSERT(n);
    //    }
