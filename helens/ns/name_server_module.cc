...
bool NameServerModule::handleRockRequest(helens::RockRequest::ptr request
                                         ,helens::RockResponse::ptr response
                                         ,helens::RockStream::ptr stream) {
    helens::Atomic::addFetch(s_request_count, 1);
    switch(request->getCmd()) {
        ...
        default:
            HELENS_LOG_WARN(g_logger) << "invalid cmd=0x" << std::hex << request->getCmd();
            break;
    }
    return true;
}

bool NameServerModule::handleRockNotify(helens::RockNotify::ptr notify
                                        ,helens::RockStream::ptr stream) {
    return true;
}

bool NameServerModule::onConnect(helens::Stream::ptr stream) {
    helens::Atomic::addFetch(s_on_connect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        HELENS_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        HELENS_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}

bool NameServerModule::onDisconnect(helens::Stream::ptr stream) {
    helens::Atomic::addFetch(s_on_disconnect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        HELENS_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    ...
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        HELENS_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
NSClientInfo::ptr NameServerModule::get(helens::RockStream::ptr rs) {
    helens::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(rs);
    return it == m_sessions.end() ? nullptr : it->second;
}

bool NameServerModule::handleRegister(helens::RockRequest::ptr request
                                      ,helens::RockResponse::ptr response
                                      ,helens::RockStream::ptr stream) {
    auto rr = request->getAsPB<RegisterRequest>();
    if(!rr) {
        HELENS_LOG_ERROR(g_logger) << "invalid register request from: "
...
void NameServerModule::set(helens::RockStream::ptr rs, NSClientInfo::ptr new_value) {
    if(!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    //helens::WorkerMgr::GetInstance()->schedule("notify",
...
std::set<helens::RockStream::ptr> NameServerModule::getStreams(const std::string& domain) {
    helens::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end() ? std::set<helens::RockStream::ptr>() : it->second;
}
...
bool NameServerModule::handleQuery(helens::RockRequest::ptr request
                                   ,helens::RockResponse::ptr response
                                   ,helens::RockStream::ptr stream) {
    auto qreq = request->getAsPB<QueryRequest>();
    if(!qreq) {
        HELENS_LOG_ERROR(g_logger) << "invalid query request from: "
            << stream->getRemoteAddressString();
        return false;
    }
...
void NameServerModule::setQueryDomain(helens::RockStream::ptr rs, const std::set<std::string>& ds) {
...
bool NameServerModule::handleTick(helens::RockRequest::ptr request
                                  ,helens::RockResponse::ptr response
                                  ,helens::RockStream::ptr stream) {
    return true;
}
