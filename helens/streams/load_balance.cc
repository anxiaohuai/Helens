...
void LoadBalanceItem::close() {
    if(m_stream) {
        ...
        helens::WorkerMgr::GetInstance()->schedule("service_io", [stream](){
            stream->close();
        });
    }
}
...
std::string LoadBalance::statusString(const std::string& prefix) {
    ...
    lock.unlock();
    std::stringstream ss;
    ss << prefix << "init_time: " << helens::Time2Str(m_lastInitTime / 1000) << std::endl;
    for(auto& i : datas) {
...
int32_t WeightLoadBalance::getIdx(uint64_t v) {
    ...
    auto it = std::upper_bound(m_weights.begin()
                ,m_weights.end(), dis);
    HELENS_ASSERT(it != m_weights.end());
...
void SDLoadBalance::onServiceChange(const std::string& domain, const std::string& service
    ...
    HELENS_LOG_INFO(g_logger) << "onServiceChange domain=" << domain
                             << " service=" << service;
    auto type = getType(domain, service);
    ...
    for(auto& i : add_values) {
        ...
        if(!stream) {
            HELENS_LOG_ERROR(g_logger) << "create stream fail, " << i.second->toString();
            continue;
        }
