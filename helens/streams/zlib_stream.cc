...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    HELENS_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    HELENS_ASSERT((window_bits >= 8 && window_bits <= 15));
    HELENS_ASSERT((memlevel >= 1 && memlevel <= 9));
...
helens::ByteArray::ptr ZlibStream::getByteArray() {
