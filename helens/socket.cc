...
Socket::ptr Socket::CreateTCP(helens::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(helens::Address::ptr address) {
...
bool Socket::getOption(int level, int option, void* result, socklen_t* len) {
    ...
    if(rt) {
        HELENS_LOG_DEBUG(g_logger) << "getOption sock=" << m_sock
...
bool Socket::setOption(int level, int option, const void* result, socklen_t len) {
    if(setsockopt(m_sock, level, option, result, (socklen_t)len)) {
        HELENS_LOG_DEBUG(g_logger) << "setOption sock=" << m_sock
            << " level=" << level << " option=" << option
            << " errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
...
Socket::ptr Socket::accept() {
    ...
    if(newsock == -1) {
        HELENS_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno="
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if(!isValid()) {
        newSock();
        if(HELENS_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if(HELENS_UNLIKELY(addr->getFamily() != m_family)) {
        HELENS_LOG_ERROR(g_logger) << "bind sock.family("
    ...
    if(uaddr) {
        ...
        if(sock->connect(uaddr)) {
            ...
            helens::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }

    if(::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        HELENS_LOG_ERROR(g_logger) << "bind error errrno=" << errno
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if(!m_remoteAddress) {
        HELENS_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if(HELENS_UNLIKELY(addr->getFamily() != m_family)) {
        HELENS_LOG_ERROR(g_logger) << "connect sock.family("
            << m_family << ") addr.family(" << addr->getFamily()
            << ") not equal, addr=" << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1) {
        if(::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
            HELENS_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if(!isValid()) {
        HELENS_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)) {
        HELENS_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if(getpeername(m_sock, result->getAddr(), &addrlen)) {
        //HELENS_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
        //    << " errno=" << errno << " errstr=" << strerror(errno);
...
Address::ptr Socket::getLocalAddress() {
    ...
    if(getsockname(m_sock, result->getAddr(), &addrlen)) {
        HELENS_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if(m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, helens::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, helens::IOManager::WRITE);
}
...
void Socket::newSock() {
    ...
    if(HELENS_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        HELENS_LOG_ERROR(g_logger) << "socket(" << m_family
...
bool SSLSocket::loadCertificates(const std::string& cert_file, const std::string& key_file) {
    ...
    if(SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
        HELENS_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
            << cert_file << ") error";
        return false;
    }
    if(SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
        HELENS_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
            << key_file << ") error";
        return false;
    }
    if(SSL_CTX_check_private_key(m_ctx.get()) != 1) {
        HELENS_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
...
SSLSocket::ptr SSLSocket::CreateTCP(helens::Address::ptr address) {
