...
static helens::ConfigVar<uint32_t>::ptr g_rock_protocol_max_length
    = helens::Config::Lookup("rock.protocol.max_length",
                            (uint32_t)(1024 * 1024 * 64), "rock protocol max length");

static helens::ConfigVar<uint32_t>::ptr g_rock_protocol_gzip_min_length
    = helens::Config::Lookup("rock.protocol.gzip_min_length",
...
bool RockRequest::serializeToByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        HELENS_LOG_ERROR(g_logger) << "RockRequest serializeToByteArray error";
    }
    return false;
}
...
bool RockRequest::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        HELENS_LOG_ERROR(g_logger) << "RockRequest parseFromByteArray error";
    }
    return false;
}
...
bool RockResponse::serializeToByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        HELENS_LOG_ERROR(g_logger) << "RockResponse serializeToByteArray error";
    }
    return false;
}
...
bool RockResponse::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        HELENS_LOG_ERROR(g_logger) << "RockResponse parseFromByteArray error";
    }
    return false;
}
...
bool RockNotify::serializeToByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        HELENS_LOG_ERROR(g_logger) << "RockNotify serializeToByteArray error";
    }
    return false;
}
...
bool RockNotify::parseFromByteArray(ByteArray::ptr bytearray) {
    try {
        ...
        HELENS_LOG_ERROR(g_logger) << "RockNotify parseFromByteArray error";
    }
    return false;
}
...
Message::ptr RockMessageDecoder::parseFrom(Stream::ptr stream) {
    try {
        ...
        if(stream->readFixSize(&header, sizeof(header)) <= 0) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder decode head error";
            return nullptr;
        }

        if(memcmp(header.magic, s_rock_magic, sizeof(s_rock_magic))) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder head.magic error";
            return nullptr;
        }

        if(header.version != 0x1) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder head.version != 0x1";
            return nullptr;
        }

        header.length = helens::byteswapOnLittleEndian(header.length);
        if((uint32_t)header.length >= g_rock_protocol_max_length->getValue()) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder head.length("
                                      << header.length << ") >="
        ...
        if(stream->readFixSize(ba, header.length) <= 0) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder read body fail length=" << header.length;
            return nullptr;
        }

        ba->setPosition(0);
        if(header.flag & 0x1) { //gizp
            auto zstream = helens::ZlibStream::CreateGzip(false);
            if(zstream->write(ba, -1) != Z_OK) {
                HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder ungzip error";
                return nullptr;
            }
            if(zstream->flush() != Z_OK) {
                HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder ungzip flush error";
                return nullptr;
            }
        ...
        switch(type) {
            ...
            case Message::NOTIFY:
                ...
                break;
            default:
                HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder invalid type=" << (int)type;
                return nullptr;
        }

        if(!msg->parseFromByteArray(ba)) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder parseFromByteArray fail type=" << (int)type;
            return nullptr;
        }
        return msg;
    } catch (std::exception& e) {
        HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder except:" << e.what();
    } catch (...) {
        HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder except";
    }
    return nullptr;
}
...
int32_t RockMessageDecoder::serializeTo(Stream::ptr stream, Message::ptr msg) {
    ...
    if((uint32_t)header.length >= g_rock_protocol_gzip_min_length->getValue()) {
        auto zstream = helens::ZlibStream::CreateGzip(true);
        if(zstream->write(ba, -1) != Z_OK) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo gizp error";
            return -1;
        }
        if(zstream->flush() != Z_OK) {
            HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo gizp flush error";
            return -2;
        }
    ...
    if(stream->writeFixSize(&header, sizeof(header)) <= 0) {
        HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo write header fail";
        return -3;
    }
    if(stream->writeFixSize(ba, ba->getReadSize()) <= 0) {
        HELENS_LOG_ERROR(g_logger) << "RockMessageDecoder serializeTo write body fail";
        return -4;
    }
