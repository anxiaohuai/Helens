...
static helens::ConfigVar<std::unordered_map<std::string
    ,std::unordered_map<std::string, std::string> > >::ptr g_rock_services =
    helens::Config::Lookup("rock_services", std::unordered_map<std::string
...
RockStream::RockStream(Socket::ptr sock)
    ...
    HELENS_LOG_DEBUG(g_logger) << "RockStream::RockStream "
        << this << " "
        << (sock ? sock->toString() : "");
}
...
RockResult::ptr RockStream::request(RockRequest::ptr req, uint32_t timeout_ms) {
    if(isConnected()) {
        ...
        addCtx(ctx);
        uint64_t ts = helens::GetCurrentMS();
        ctx->timer = helens::IOManager::GetThis()->addTimer(timeout_ms,
                std::bind(&RockStream::onTimeOut, shared_from_this(), ctx));
        enqueue(ctx);
        helens::Fiber::YieldToHold();
        return std::make_shared<RockResult>(ctx->result, helens::GetCurrentMS() - ts, ctx->response, req);
    } else {
...
AsyncSocketStream::Ctx::ptr RockStream::doRecv() {
    //HELENS_LOG_INFO(g_logger) << "doRecv " << this;
    ...
    if(type == Message::RESPONSE) {
        ...
        if(!rsp) {
            HELENS_LOG_WARN(g_logger) << "RockStream doRecv response not RockResponse: "
                << msg->toString();
            return nullptr;
        }
        RockCtx::ptr ctx = getAndDelCtxAs<RockCtx>(rsp->getSn());
        if(!ctx) {
            HELENS_LOG_WARN(g_logger) << "RockStream request timeout reponse="
                << rsp->toString();
            return nullptr;
        }
        ...
        if(!req) {
            HELENS_LOG_WARN(g_logger) << "RockStream doRecv request not RockRequest: "
                << msg->toString();
            return nullptr;
        }
        if(m_requestHandler) {
            ...
            HELENS_LOG_WARN(g_logger) << "unhandle request " << req->toString();
        }
    } else if(type == Message::NOTIFY) {
        auto nty = std::dynamic_pointer_cast<RockNotify>(msg);
        if(!nty) {
            HELENS_LOG_WARN(g_logger) << "RockStream doRecv notify not RockNotify: "
                << msg->toString();
            return nullptr;
        }
        ...
        if(m_notifyHandler) {
            ...
            HELENS_LOG_WARN(g_logger) << "unhandle notify " << nty->toString();
        }
    } else {
        HELENS_LOG_WARN(g_logger) << "RockStream recv unknow type=" << type
            << " msg: " << msg->toString();
    }
    return nullptr;
}

void RockStream::handleRequest(helens::RockRequest::ptr req) {
    helens::RockResponse::ptr rsp = req->createResponse();
...
void RockStream::handleNotify(helens::RockNotify::ptr nty) {
...
bool RockConnection::connect(helens::Address::ptr addr) {
    m_socket = helens::Socket::CreateTCP(addr);
...
static SocketStream::ptr create_rock_stream(ServiceItemInfo::ptr info) {
    helens::IPAddress::ptr addr = helens::Address::LookupAnyIPAddress(info->getIp());
    if(!addr) {
        HELENS_LOG_ERROR(g_logger) << "invalid service info: " << info->toString();
        return nullptr;
    }
    ...
    helens::WorkerMgr::GetInstance()->schedule("service_io", [conn, addr](){
        conn->connect(addr);
        conn->start();
    });
    return conn;
...
RockResult::ptr RockSDLoadBalance::request(const std::string& domain, const std::string& service,
    ...
    uint64_t ts2 = helens::GetCurrentMS();
    if(r->result == 0) {
        stats.incOks(1);
