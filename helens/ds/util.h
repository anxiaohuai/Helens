#ifndef __HELENS_DS_UTIL_H__
#define __HELENS_DS_UTIL_H__
...
public:
    uint32_t operator()(const T& data) {
        return helens::murmur3_hash(&data, sizeof(T), seed);
    }
};
...
public:
    uint32_t operator()(const std::string& data) {
        return helens::murmur3_hash(data.data(), data.size(), seed);
    }
};
...
public:
    uint32_t operator()(const std::vector<T>& data) {
        return helens::murmur3_hash(&data[0], data.size() * sizeof(T), seed);
    }
};
...
public:
    uint64_t operator()(const T& data) {
        return helens::murmur3_hash64(&data, sizeof(T), seed, seed2);
    }
};
...
public:
    uint64_t operator()(const std::string& data) {
        return helens::murmur3_hash64(data.data(), data.size(), seed, seed2);
    }
};
...
public:
    uint32_t operator()(const std::vector<T>& data) {
        return helens::murmur3_hash64(&data[0], data.size() * sizeof(T), seed, seed2);
    }
};
