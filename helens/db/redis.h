#ifndef __HELENS_DB_REDIS_H__
#define __HELENS_DB_REDIS_H__
...
public:
    ...
    FoxRedis(helens::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
private:
    struct FCtx {
        std::string cmd;
        helens::Scheduler* scheduler;
        helens::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
public:
    ...
    FoxRedisCluster(helens::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef helens::Singleton<RedisManager> RedisMgr;
