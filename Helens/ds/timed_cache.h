#ifndef __HELENS_DS_TIMED_CACHE_H__
#define __HELENS_DS_TIMED_CACHE_H__
...
template<class K, class V, class RWMutexType = helens::RWMutex>
class TimedCache {
private:
    struct Item {
...
public:
    ...
    void set(const K& k, const V& v, uint64_t expired) {
        ...
        auto sit = m_timed.insert(Item(k, v, expired + helens::GetCurrentMS()));
    ...
    bool expired(const K& k, const uint64_t& ts) {
        ...
        uint64_t tts = ts + helens::GetCurrentMS();
        if(it->second->ts == tts) {
            return true;
        }
        auto item = *it->second;
...
template<class K, class V, class RWMutexType = helens::RWMutex, class Hash = std::hash<K> >
