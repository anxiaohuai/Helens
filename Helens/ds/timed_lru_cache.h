#ifndef __HELENS_DS_TIMED_LRU_CACHE_H__
#define __HELENS_DS_TIMED_LRU_CACHE_H__
...
template<class K, class V, class MutexType = helens::Mutex>
class TimedLruCache {
private:
    struct Item {
...
public:
    ...
    void set(const K& k, const V& v, uint64_t expired) {
        ...
        if(it != m_cache.end()) {
            ...
            it->second->val = v;
            it->second->ts = expired + helens::GetCurrentMS();
            m_timed.insert(it->second);
            return;
        }

        m_keys.emplace_front(Item(k, v, expired + helens::GetCurrentMS()));
    ...
    size_t checkTimeout(const uint64_t& ts = helens::GetCurrentMS()) {
        size_t size = 0;
        typename MutexType::Lock lock(m_mutex);
...
template<class K, class V, class MutexType = helens::Mutex, class Hash = std::hash<K> >
