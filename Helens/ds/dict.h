#ifndef __HELENS_DS_DICT_H__
#define __HELENS_DS_DICT_H__

#include "helens/ds/util.h"
...
template<class K
        ,class V
        ,class PosHash = helens::ds::Murmur3Hash<K>
        >
class Dict {
...
public:
    ...
    SharedArray<V> get(const K& k, bool duplicate = true) {
        ...
        helens::RWMutex::ReadLock lock2(s_mutex[pos % MAX_MUTEX]);
        auto it = BinarySearch(m_datas[pos].begin(), m_datas[pos].end(), Node(k));
        //HELENS_ASSERT(it == std::find(m_datas[pos].begin(), m_datas[pos].end(), Node(k)));
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        helens::RWMutex::ReadLock lock(m_mutex);
        uint32_t pos = hashvalue % m_size;
        helens::RWMutex::WriteLock lock2(s_mutex[pos % MAX_MUTEX]);
        ...
        if(it == m_datas[pos].end()) {
            ...
            helens::Atomic::addFetch(m_total);
        } else {
            if(it->size == (int)size) {
    ...
    void foreach(callback cb) {
        ...
        for(size_t i = 0; i < m_size; ++i) {
            helens::RWMutex::ReadLock lock2(s_mutex[i % MAX_MUTEX]);
    ...
    bool del(const K& k) {
        ...
        if(it == m_datas[pos].end()) {
            ...
            std::sort(m_datas[pos].begin(), m_datas[pos].end());
            helens::Atomic::subFetch(m_total);
        }
        return true;
    }
    ...
    bool writeTo(std::ostream& os, uint64_t speed = -1) {
        ...
        if(speed == (uint64_t)-1) {
            ...
            helens::WriteFixToStreamWithSpeed(os, (const char*)&vs[0], size, speed);
        }
        ...
        if(speed == (uint64_t)-1) {
            ...
            helens::WriteFixToStreamWithSpeed(os, (const char*)&ns[0], size, speed);
        }
...
private:
    ...
    static helens::RWMutex s_mutex[MAX_MUTEX];
};
...
template<class K
        ...
        ,class PosHash
        >
helens::RWMutex Dict<K, V, PosHash>::s_mutex[MAX_MUTEX];
...
public:
    static uint64_t GetID(const std::string& str) {
        ...
        return helens::murmur3_hash64(str.c_str(), str.size(), 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str, const uint32_t& size) {
        if(size == 0) {
            return 0;
        }
        return helens::murmur3_hash64(str, size, 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str) {
        if(str == nullptr) {
            return 0;
        }
        return helens::murmur3_hash64(str, 1060627423, 1050126127);
    }
