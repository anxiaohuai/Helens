#ifndef __HELENS_DS_HASH_MULTIMAP_H__
#define __HELENS_DS_HASH_MULTIMAP_H__
...
public:
    ...
    bool insert(const K& k, const V& v) {
        ...
        if(it == m_datas[pos].end()) {
            ...
            helens::Atomic::addFetch(m_elements);
        } else {
            auto iit = BinarySearch(it->val, it->val + it->size, v);
            if(iit != it->val + it->size) {
                *iit = v;
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        if(it == m_datas[pos].end()) {
            ...
            helens::Atomic::addFetch(m_total);
            helens::Atomic::addFetch(m_elements, (uint64_t)size);
        } else {
            std::set<V> news;
            ...
            it->val = datas;
            helens::Atomic::addFetch(m_elements, news.size());
        }
        return true;
    }
    ...
    bool del(const K& k) {
        ...
        if(it == m_datas[pos].end()) {
            ...
            helens::Atomic::subFetch(m_elements, (uint64_t)it->size);
    ...
    bool del(const K& k, const V& v) {
        ...
        if(it == m_datas[pos].end()) {
            ...
            if(it->size == 1) {
                ...
                helens::Atomic::subFetch(m_total);
                helens::Atomic::subFetch(m_elements);
                return true;
            }
...
template<class K
        ...
        ,class PosHash
        >
helens::RWMutex HashMultimap<K, V, PosHash>::s_mutex[MAX_MUTEX];
