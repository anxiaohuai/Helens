...
static helens::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    helens::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2),
            "tcp server read timeout");

static helens::Logger::ptr g_logger = HELENS_LOG_NAME("system");

TcpServer::TcpServer(helens::IOManager* worker,
...
bool TcpServer::bind(helens::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs
    ...
    for(auto& addr : addrs) {
        ...
        if(!sock->bind(addr)) {
            HELENS_LOG_ERROR(g_logger) << "bind fail errno="
        ...
        if(!sock->listen()) {
            HELENS_LOG_ERROR(g_logger) << "listen fail errno="
                << errno << " errstr=" << strerror(errno)
                << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
    ...
    for(auto& i : m_socks) {
        HELENS_LOG_INFO(g_logger) << "type=" << m_type
            << " name=" << m_name
            << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock) {
    while(!m_isStop) {
        ...
        if(client) {
            ...
            HELENS_LOG_ERROR(g_logger) << "accept errno=" << errno
