#ifndef __HELENS_WORKER_H__
#define __HELENS_WORKER_H__
...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, helens::Scheduler* s = helens::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, helens::Scheduler* s = helens::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static helens::Logger::ptr s_logger = HELENS_LOG_NAME("system");
            HELENS_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef helens::Singleton<WorkerManager> WorkerMgr;
