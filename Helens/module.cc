...
static helens::ConfigVar<std::string>::ptr g_module_path
...
bool Module::handleRequest(helens::Message::ptr req
                           ,helens::Message::ptr rsp
                           ,helens::Stream::ptr stream) {
    HELENS_LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString()
            << " rsp=" << rsp->toString() << " stream=" << stream;
    return true;
}

bool Module::handleNotify(helens::Message::ptr notify
                          ,helens::Stream::ptr stream) {
    HELENS_LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString()
            << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(helens::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(helens::Stream::ptr stream) {
    return true;
}
...
bool RockModule::handleRequest(helens::Message::ptr req
                               ,helens::Message::ptr rsp
                               ,helens::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<helens::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<helens::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<helens::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(helens::Message::ptr notify
                              ,helens::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<helens::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<helens::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    helens::FSUtil::ListAllFile(files, path, ".so");
