...
helens::ConfigVar<uint32_t>::ptr g_websocket_message_max_size
    = helens::Config::Lookup("websocket.message.max_size"
...
HttpRequest::ptr WSSession::handleShake() {
    ...
    do {
        req = recvRequest();
        if(!req) {
            HELENS_LOG_INFO(g_logger) << "invalid http request";
            break;
        }
        if(strcasecmp(req->getHeader("Upgrade").c_str(), "websocket")) {
            HELENS_LOG_INFO(g_logger) << "http header Upgrade != websocket";
            break;
        }
        if(strcasecmp(req->getHeader("Connection").c_str(), "Upgrade")) {
            HELENS_LOG_INFO(g_logger) << "http header Connection != Upgrade";
            break;
        }
        if(req->getHeaderAs<int>("Sec-webSocket-Version") != 13) {
            HELENS_LOG_INFO(g_logger) << "http header Sec-webSocket-Version != 13";
            break;
        }
        std::string key = req->getHeader("Sec-WebSocket-Key");
        if(key.empty()) {
            HELENS_LOG_INFO(g_logger) << "http header Sec-WebSocket-Key = null";
            break;
        }

        std::string v = key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
        v = helens::base64encode(helens::sha1sum(v));
        ...
        sendResponse(rsp);
        HELENS_LOG_DEBUG(g_logger) << *req;
        HELENS_LOG_DEBUG(g_logger) << *rsp;
        return req;
    } while(false);
    if(req) {
        HELENS_LOG_INFO(g_logger) << *req;
    }
    return nullptr;
}
...
WSFrameMessage::ptr WSRecvMessage(Stream* stream, bool client) {
    ...
    do {
        ...
        HELENS_LOG_DEBUG(g_logger) << "WSFrameHead " << ws_head.toString();

        if(ws_head.opcode == WSFrameHead::PING) {
            HELENS_LOG_INFO(g_logger) << "PING";
            if(WSPong(stream) <= 0) {
                break;
            }
            ...
            if(!client && !ws_head.mask) {
                HELENS_LOG_INFO(g_logger) << "WSFrameHead mask != 1";
                break;
            }
            uint64_t length = 0;
            if(ws_head.payload == 126) {
                ...
                length = helens::byteswapOnLittleEndian(len);
            ...
            if((cur_len + length) >= g_websocket_message_max_size->getValue()) {
                HELENS_LOG_WARN(g_logger) << "WSFrameMessage length > "
            ...
            if(ws_head.fin) {
                HELENS_LOG_DEBUG(g_logger) << data;
                return WSFrameMessage::ptr(new WSFrameMessage(opcode, std::move(data)));
            }
        } else {
            HELENS_LOG_DEBUG(g_logger) << "invalid opcode=" << ws_head.opcode;
        }
    } while(true);
    stream->close();
    return nullptr;
...
int32_t WSSendMessage(Stream* stream, WSFrameMessage::ptr msg, bool client, bool fin) {
    do {
        ...
        if(ws_head.payload == 126) {
            uint16_t len = size;
            len = helens::byteswapOnLittleEndian(len);
            if(stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        } else if(ws_head.payload == 127) {
            uint64_t len = helens::byteswapOnLittleEndian(size);
            if(stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        }
