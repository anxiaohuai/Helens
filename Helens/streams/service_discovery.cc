...
ServiceItemInfo::ptr ServiceItemInfo::Create(const std::string& ip_and_port, const std::string& data) {
    ...
    auto port = helens::TypeUtil::Atoi(ip_and_port.substr(pos + 1));
...
void ZKServiceDiscovery::start() {
    ...
    m_client.reset(new helens::ZKClient);
    bool b = m_client->init(m_hosts, 6000, std::bind(&ZKServiceDiscovery::onWatch,
                self, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3, std::placeholders::_4));
    if(!b) {
        HELENS_LOG_ERROR(g_logger) << "ZKClient init fail, hosts=" << m_hosts;
    }
    m_timer = helens::IOManager::GetThis()->addTimer(60 * 1000, [self, this](){
        m_isOnTimer = true;
...
void ZKServiceDiscovery::onZKConnect(const std::string& path, ZKClient::ptr client) {
    ...
    if(!ok) {
        HELENS_LOG_ERROR(g_logger) << "onZKConnect register fail";
    }
    ...
    if(!ok) {
        HELENS_LOG_ERROR(g_logger) << "onZKConnect query fail";
    }
    ...
    if(!ok) {
        HELENS_LOG_ERROR(g_logger) << "onZKConnect queryData fail";
    }
}
...
bool ZKServiceDiscovery::existsOrCreate(const std::string& path) {
    ...
    if(v == ZOK) {
        ...
        if(pos == std::string::npos) {
            HELENS_LOG_ERROR(g_logger) << "existsOrCreate invalid path=" << path;
            return false;
        }
        if(pos == 0 || existsOrCreate(path.substr(0, pos))) {
            ...
            if(v != ZOK) {
                HELENS_LOG_ERROR(g_logger) << "create path=" << path << " error:"
                    << zerror(v) << " (" << v << ")";
                return false;
            }
            return true;
...
bool ZKServiceDiscovery::registerInfo(const std::string& domain, const std::string& service, 
    ...
    if(!v) {
        HELENS_LOG_ERROR(g_logger) << "create path=" << path << " fail";
        return false;
    }
    ...
    if(!m_isOnTimer) {
        HELENS_LOG_ERROR(g_logger) << "create path=" << (path + "/" + ip_and_port) << " fail, error:"
            << zerror(rt) << " (" << rt << ")";
    }
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain, const std::string& service) {
    if(service != "all") {
        ...
        if(m_selfInfo.empty()) {
            HELENS_LOG_ERROR(g_logger) << "queryInfo selfInfo is null";
            return false;
        }
        ...
        if(!m_isOnTimer) {
            HELENS_LOG_ERROR(g_logger) << "create path=" << (path + "/" + m_selfInfo) << " fail, error:"
                << zerror(rt) << " (" << rt << ")";
        }
        return rt == ZNODEEXISTS;
    } else {
...
bool ZKServiceDiscovery::getChildren(const std::string& path) {
    ...
    if(!ParseDomainService(path, domain, service)) {
        HELENS_LOG_ERROR(g_logger) << "get_children path=" << path
            << " invalid path";
        return false;
    }
    {
        ...
        if(it->second.count(service) == 0
            ...
            HELENS_LOG_ERROR(g_logger) << "get_children path=" << path
                ...
                << helens::Join(it->second.begin(), it->second.end(), ",");
            return false;
        }
    }
    ...
    if(v != ZOK) {
        HELENS_LOG_ERROR(g_logger) << "get_children path=" << path << " fail, error:"
            << zerror(v) << " (" << v << ")";
        return false;
    }
    std::unordered_map<uint64_t, ServiceItemInfo::ptr> infos;
    for(auto& i : vals) {
        ...
        HELENS_LOG_INFO(g_logger) << "domain=" << domain
...
bool ZKServiceDiscovery::queryData(const std::string& domain, const std::string& service) {
    //HELENS_LOG_INFO(g_logger) << "query_data domain=" << domain
    //                         << " service=" << service;
    if(service != "all") {
        std::string path = GetProvidersPath(domain, service);
...
void ZKServiceDiscovery::onWatch(int type, int stat, const std::string& path, ZKClient::ptr client) {
    ...
    HELENS_LOG_ERROR(g_logger) << "onWatch hosts=" << m_hosts
        << " type=" << type << " stat=" << stat
