...
void AsyncSocketStream::Ctx::doRsp() {
    ...
    if(!helens::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler*)nullptr)) {
        return;
    }
    if(!scd || !fiber) {
        return;
...
bool AsyncSocketStream::start() {
    if(!m_iomanager) {
        m_iomanager = helens::IOManager::GetThis();
    }
    if(!m_worker) {
        m_worker = helens::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead() {
    ...
    HELENS_LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite() {
    ...
    HELENS_LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
bool AsyncSocketStream::innerClose() {
    HELENS_ASSERT(m_iomanager == helens::IOManager::GetThis());
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get() {
    ...
    for(uint32_t i = 0; i < m_size; ++i) {
        auto idx = helens::Atomic::addFetch(m_idx, 1);
