#ifndef __HELENS_STREAMS_SOCKET_STREAM_POOL_H__
#define __HELENS_STREAMS_SOCKET_STREAM_POOL_H__

#include "helens/streams/socket_stream.h"
...
public:
    ...
    uint32_t incUsedTime(uint32_t v) { return helens::Atomic::addFetch(m_usedTime ,v);}
    uint32_t incTotal(uint32_t v) { return helens::Atomic::addFetch(m_total, v);}
    uint32_t incDoing(uint32_t v) { return helens::Atomic::addFetch(m_doing, v);}
    uint32_t incTimeouts(uint32_t v) { return helens::Atomic::addFetch(m_timeouts, v);}
    uint32_t incOks(uint32_t v) { return helens::Atomic::addFetch(m_oks, v);}
    uint32_t incErrs(uint32_t v) { return helens::Atomic::addFetch(m_errs, v);}

    uint32_t decDoing(uint32_t v) { return helens::Atomic::subFetch(m_doing, v);}
    void clear();
...
public:
    typedef helens::RWMutex RWMutexType;
