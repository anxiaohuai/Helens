...
std::string Table::getFilename() const {
    return helens::ToLower(m_name + m_subfix);
}

bool Table::init(const tinyxml2::XMLElement& node) {
    if(!node.Attribute("name")) {
        HELENS_LOG_ERROR(g_logger) << "table name is null";
        return false;
    }

    m_name = node.Attribute("name");
    if(!node.Attribute("namespace")) {
        HELENS_LOG_ERROR(g_logger) << "table namespace is null";
        return false;
    }
    ...
    if(!cols) {
        HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " columns is null";
        return false;
    }

    const tinyxml2::XMLElement* col = cols->FirstChildElement("column");
    if(!col) {
        HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " column is null";
        return false;
    }
    ...
    do {
        ...
        if(!col_ptr->init(*col)) {
            HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " init column error";
            return false;
        }
        if(col_names.insert(col_ptr->getName()).second == false) {
            HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " column name="
    ...
    if(!idxs) {
        HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " indexs is null";
        return false;
    }

    const tinyxml2::XMLElement* idx = idxs->FirstChildElement("index");
    if(!idx) {
        HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " index is null";
        return false;
    }
    ...
    do {
        ...
        if(!idx_ptr->init(*idx)) {
            HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " index init error";
            return false;
        }
        if(idx_names.insert(idx_ptr->getName()).second == false) {
            HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " index name="
        ...
        if(idx_ptr->isPK()) {
            if(has_pk) {
                HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " more than one pk"; 
                return false;
            }
            has_pk = true;
        }
        ...
        for(auto& x : cnames) {
            if(col_names.count(x) == 0) {
                HELENS_LOG_ERROR(g_logger) << "table name=" << m_name << " idx="
...
void Table::gen(const std::string& path) {
    std::string p = path + "/" + helens::replace(m_namespace, ".", "/");
    helens::FSUtil::Mkdir(p);
    gen_inc(p);
    gen_src(p);
}
...
void Table::gen_inc(const std::string& path) {
    ...
    std::set<std::string> incs = {"helens/db/db.h", "helens/util.h"};
    for(auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    std::vector<std::string> ns = helens::split(m_namespace, '.');
...
std::string Table::genToStringSrc(const std::string& class_name) {
    ...
    for(auto it = m_cols.begin();
        ...
        if((*it)->getDType() == Column::TYPE_UINT64
            ...
            ss << "helens::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
               << ";" << std::endl;
        } else {
    ...
    ss << "    return helens::JsonUtil::ToString(jvalue);" << std::endl;
    ss << "}" << std::endl;
    return ss.str();
}
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"helens/log.h\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static helens::Logger::ptr g_logger = HELENS_LOG_NAME(\"orm\");" << std::endl;
...
#define CHECK_STMT(v) \
    ...
    ofs << "        HELENS_LOG_ERROR(g_logger) << \"stmt=\" << sql" << std::endl << \
        "                 << \" errno=\"" \
