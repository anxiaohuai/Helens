...
void gen_cmake(const std::string& path, const std::map<std::string, helens::orm::Table::ptr>& tbs) {
    ...
    for(auto& i : tbs) {
        ofs << "    " << helens::replace(i.second->getNamespace(), ".", "/")
            << "/" << helens::ToLower(i.second->getFilename()) << ".cc" << std::endl;
    }
    ofs << ")" << std::endl;
...
int main(int argc, char** argv) {
    ...
    helens::FSUtil::ListAllFile(files, input_path, ".xml");
    std::vector<helens::orm::Table::ptr> tbs;
    bool has_error = false;
    for(auto& i : files) {
        HELENS_LOG_INFO(g_logger) << "init xml=" << i << " begin";
        tinyxml2::XMLDocument doc;
        if(doc.LoadFile(i.c_str())) {
            HELENS_LOG_ERROR(g_logger) << "error: " << doc.ErrorStr();
            has_error = true;
        } else {
            helens::orm::Table::ptr table(new helens::orm::Table);
            if(!table->init(*doc.RootElement())) {
                HELENS_LOG_ERROR(g_logger) << "table init error";
                has_error = true;
            } else {
                tbs.push_back(table);
            }
        }
        HELENS_LOG_INFO(g_logger) << "init xml=" << i << " end";
    }
    if(has_error) {
        return 0;
    }

    std::map<std::string, helens::orm::Table::ptr> orm_tbs;
    for(auto& i : tbs) {
        i->gen(out_path);
        orm_tbs[i->getName()] = i;
    }
